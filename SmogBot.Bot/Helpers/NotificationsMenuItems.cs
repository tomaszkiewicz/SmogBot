    ...
    public class NotificationsMenuItems : Dictionary<string, Func<IDialogContext, ResumeAfter<object>, Task>>
    {
        ...
        public NotificationsMenuItems(BotAccessor accessor)
        {
            ...
            foreach (var time in _notificationTimes)
                Add($"Usuń powiadomienie o {time}", async (ctx, resume) => await DeleteNotification(ctx, resume, time));

            Add("Menu główne", Exit);
        ...
        private async Task DeleteNotification(IDialogContext context, ResumeAfter<object> resume, string time)
        {
            ...
            await context.PostAsync($"Usunięto powiadomienie o {time}");
        ...
        {
            context.Call(new PromptTimeDialog("Podaj godzinę (np. 8:30) o której chcesz być informowany o ew. przekroczeniach norm."), (new ResumeHelper(resume, OnAddNofiticationTimeProvided)).Resume);
